...
# 删除光标所在的单词
#d i w			#i 表示在某某中间，比如这里是在 word 的中间
...
vim允许用户定义自己的标记，以便于方便跳转（从一个函数跳转到另一个函数，再跳回来）；

1、设置标记：
          使用m命令来设置标记，如在normal模式下，按下ms，则设置了一个名字为s的标记，

2、跳转到标记
          接上，在normal模式下，按下's，则会跳转到a标记的位置（建议移动一下光标再执行此命令，比如，执行G命令跳转到文末尾）

```shell
# 比如需要在A这个地方设置一个标记，在normal模式下输入“ms” (这个s是我自己定义的，可以根据A的语义来定义)；我们可以连续的设置标记，接着假设我们光标移动到了B这
个地方，我们可以“mb", 依次我们可以设置多个这样的标记

# 经过上面设置的标记，如何设置跳转到我们设置的指定的标记上呢？比如我们想跳转到A这个地方，那么我们输入  's  

# 注意

’‘ 可以来回跳，’mark (比如s) 可以跳转到指定的标记
...
3、查看标记
          当设置了很多标记时，可能记不清楚设置的标记了，那么可以输入:marks来查看设置了哪些标记
